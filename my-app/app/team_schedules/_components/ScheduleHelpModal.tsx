...
export function ScheduleHelpContent({ className, prefix }: { className?: string; prefix?: string }) {
  ...
  return (
    <p className={className}>
      ...
      【チーム単位管理】{"\n"}管理者が1列でチーム全体の予定をまとめて入力します。{ok}が活動可能・{ng}が詰みの日です。{"\n\n"}
      【メンバー管理】{"\n"}メンバーごとに回答し、{ok}数が必要人数以上かつ相手が空いている日が「成立」。{ng}が増えて必要人数に届かない確定の日は行を薄く表示し、相手の不可セルも薄く表示します。時間は自由記入のため、{ok}数は時間の重なりまでは見ていません。{"\n\n"}
      【他チームを表示】{"\n"}他チームを選んで、互いに{ok}なスクリムできる人を探そう。
    </p>
  )
}
