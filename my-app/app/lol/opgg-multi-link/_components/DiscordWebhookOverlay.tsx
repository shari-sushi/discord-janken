...
export function DiscordWebhookOverlay({ onConfirm }: { onConfirm: (url: string) => void }) {
  ...
  return (
    <div className="bg-zinc-900 border border-zinc-700 rounded-lg w-120 max-w-[90vw] p-5">
      ...
      <div className="bg-blue-950 border border-blue-800 rounded p-3 mb-4 text-xs text-blue-300">🔒 この URL はブラウザの LocalStorage にのみ保存されます。サーバーには送信されません。</div>
      ...
      <div className="relative mb-3">
        <input
          ...
          className="w-full bg-zinc-800 border border-zinc-600 text-white rounded px-3 py-2 pr-8 text-sm"
          onKeyDown={(e) => {
        ...
        {url && (
          <button
            ...
            className="absolute right-2 top-1/2 -translate-y-1/2 text-zinc-400 hover:text-white leading-none cursor-pointer"
            aria-label="URLを削除"
          >
            ✕
          </button>
      ...
      <div className="flex gap-2 justify-end">
        <button onClick={handleSave} className="bg-blue-700 hover:bg-blue-600 text-white font-semibold px-4 py-2 rounded text-sm">
          保存
        </button>
