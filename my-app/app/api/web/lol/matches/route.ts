...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!isRoleSelectValidation.valid) {
      return NextResponse.json(
        { success: false, error: `isRoleSelect: ${isRoleSelectValidation.error}` },
        { status: 400 }
      )
    }
    ...
    if (!isProtectFlag && !isRoleSelectFlag) {
      return NextResponse.json(
        { success: false, error: "isProtect または isRoleSelect の少なくとも一方を true にする必要があります" },
        { status: 400 }
      )
    }
    ...
    if (isRoleSelectFlag && !members) {
      return NextResponse.json(
        { success: false, error: "isRoleSelect: true で members なしの機能は未実装です" },
        { status: 400 }
      )
    }
    ...
    if (members) {
      ...
      if (!membersValidation.valid) {
        return NextResponse.json(
          { success: false, error: `members: ${membersValidation.error}` },
          { status: 400 }
        )
      }
    }
